...
her1 = horzcat(wsc416_RI_TB_yfp(1:96,:));

Starting_point_TB(3,1) = Starting_point_TB(3,1);
...
Starting_point_TB = vertcat(Starting_point_TB,Starting_point_TB);

End_point_TB = vertcat(End_point_TB,End_point_TB);
...
% Starting_point_TB = Starting_point_TB-1;
% 
% End_point_TB = End_point_TB-1;
...
[max_val,loc_max] = findpeaks(her1(Starting_point_TB(i,1):End_point_TB(i,1),i),'MinPeakProminence',Min_peak);    

[min_val,loc_min] = findpeaks(-her1(Starting_point_TB(i,1):End_point_TB(i,1),i),'MinPeakProminence',Min_peak);
...
for j = 1 : m
   ...
   c(j,:) = polyfit(time(Starting_point_TB(i,1)+loc_max(j,1)-2:Starting_point_TB(i,1)+loc_max(j,1),1),her1(Starting_point_TB(i,1)+loc_max(j,1)-2:Starting_point_TB(i,1)+loc_max(j,1),i),2);
...
for j = 1 : m
   ...
   k(j,:) = polyfit(time(Starting_point_TB(i,1)+loc_min(j,1)-2:Starting_point_TB(i,1)+loc_min(j,1),1),-her1(Starting_point_TB(i,1)+loc_min(j,1)-2:Starting_point_TB(i,1)+loc_min(j,1),i),2);
...
Last_Peak_time_vec(i,1) = time(loc_max(end,1))+time(Starting_point_TB(i,1),1);  
Last_Peak_value_vec(i,1) = max_val(end,1);
Second_Last_Peak_time_vec(i,1) = time(loc_max(end-1,1))+time(Starting_point_TB(i,1),1);
...
Max_Peak_time_vec(i,1) = time(loc_max(I,1),1)+time(Starting_point_TB(i,1),1); 
Integral_max_peak_vec(i,1) = sum(her1(Starting_point_TB(i,1):loc_max(I,1),i));
Integral_vec(i,1)= sum(her1(Starting_point_TB(i,1):End_point_TB(i,1),i));
