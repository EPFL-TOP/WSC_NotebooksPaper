...
class peak_detection:
    def __init__(self, x, window, th_val, th_val_, dist_val, dist_val_, prominence_val, prominence_val_, width_val, width_val_, analysis ):
        ...
        if np.size(self.peaks) ==0 or np.size(self.peaks_) ==0:
            ...
            # Peaks analysis if said by user
...
class bulk_peak_analysis():
    def __init__(self, cycles_T_minus_plot, cycles_T_plus_plot, cycles_A_minus_plot, cycles_A_plus_plot):
        ...
        for i in range(len(cycles_T_minus_plot)):
            self.mean_T_minus_plot[i] = np.nanmedian(cycles_T_minus_plot[i])
            self.std_T_minus_plot[i] = np.nanstd(cycles_T_minus_plot[i])
            self.mean_T_plus_plot[i] = np.nanmedian(cycles_T_plus_plot[i])
            self.std_T_plus_plot[i] = np.nanstd(cycles_T_plus_plot[i])
            self.mean_A_minus_plot[i] = np.nanmedian(cycles_A_minus_plot[i])
            self.std_A_minus_plot[i] = np.nanstd(cycles_A_minus_plot[i])
            self.mean_A_plus_plot[i] = np.nanmedian(cycles_A_plus_plot[i])
            self.std_A_plus_plot[i] = np.nanstd(cycles_A_plus_plot[i])
