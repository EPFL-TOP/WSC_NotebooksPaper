    ...
    def __init__(self, path_csv, path_xml):
        ...
        for i in range(n_features):
            if data.keys()[i][0]=='Track N spots':
                self.n_spots = np.array(data[data.keys()[i]][1:], dtype=int)
                break
        ind +=2
        ...
        if len(data.keys())==ind:
            ...
            for i in range(len(data.keys())-ind):
                if len(data.keys()[ind+i]):
                    ...
                    self.tags_all.append(data[data.keys()[ind+i]])
                    
        # Save tags and subtags as two different arrays
        self.tag = []
        self.subtag = []
        for i in range(len(self.tags)):
            self.tag.append(self.tags[i][0])
            self.subtag.append(self.tags[i][1])
            
        # Organize Tags and Subtags
        # Find unique tags
        n_tags = np.unique(self.tag)

        self.tags_all_save = {key:[] for key in n_tags}

        for j,val in enumerate(n_tags):

            # For each tag, find all the subtags
            ind_st = np.where(np.array(self.tag) == val)[0]

            # Make a list where the subtags will be saved

            # 1. give them a count depending on the subtag
            subtag_list = np.zeros(len(self.tags_all[j][1:]))
            count = 1
            for i in ind_st:
                inds = np.where(self.tags_all[i][1:]==1)[0]
                subtag_list[inds] = count
                count += 1

            # 2. Instead of the number, save the subtag name  
            aux_list = list(subtag_list)
            count = 1
            for k in ind_st:
                subtag = self.tags_all[k][1:].name[1]
                aux_list = [subtag if elem == count else elem for elem in aux_list]
                count +=1

            self.tags_all_save[val] = np.array(aux_list)
...
class ordering_tracks:
    def __init__(self, path_csv, path_xml):
        ...
        for i in range(len(cells_track)):
            # Distinguish between divisions and cells which were manually skipped
            # during the tracking process --> These cells will have more than 2 links

            # Are there any divisions in the track?  
            # (e.g. the spot divides in two different opportunities during all the timeseries)
            n_divs_aux = len(list(map(int,np.where(links_by_track[i]>2)[0])))
            val0_save = np.zeros(n_divs_aux)
            
            if n_divs_aux>0:
                divs_aux = np.where(links_by_track[i]>2)[0]
                for ind_, div in enumerate(divs_aux):

                    # If once the cell divided it did not move, its a skipped cell.
                    # Otherwise, it divided
                    if np.abs((pos_by_track[0][i][div+1]-pos_by_track[0][i][div+1]))>0:
                        val0_save[ind_] = div

            val0_save = val0_save[val0_save>0]


            n_divs = len(val0_save)
...
class peak_detection:
    def __init__(self, x, window, th_val, th_val_, dist_val, dist_val_, prominence_val, prominence_val_, width_val, width_val_,analysis ):
        ...
        # Maxima
        self.peaks = []
        self.peaks, _ = signal.find_peaks(cA, prominence=prominence_val, width=width_val, distance=dist_val, threshold=th_val)
        self.peaks = np.unique(self.peaks)
        # Minima
        self.peaks_ = []
        self.peaks_, _ = signal.find_peaks(cA_, prominence=prominence_val_, width=width_val_, distance=dist_val_, threshold=th_val_)
...
class bulk_peak_analysis():
    def __init__(self, cycles_T_minus_plot, cycles_T_plus_plot, cycles_A_minus_plot, cycles_A_plus_plot):
        ...
        self.mean_T_minus_plot = np.zeros(len(cycles_T_minus_plot))
        self.std_T_minus_plot = np.zeros(len(cycles_T_minus_plot))
        self.mean_T_plus_plot = np.zeros(len(cycles_T_plus_plot))
        self.std_T_plus_plot = np.zeros(len(cycles_T_plus_plot))
        self.mean_A_minus_plot = np.zeros(len(cycles_A_minus_plot))
        self.std_A_minus_plot = np.zeros(len(cycles_A_minus_plot))
        self.mean_A_plus_plot = np.zeros(len(cycles_A_plus_plot))
        self.std_A_plus_plot = np.zeros(len(cycles_A_plus_plot))

        for i in range(len(cycles_T_minus_plot)):
            self.mean_T_minus_plot[i] = np.median(cycles_T_minus_plot[i])
            self.std_T_minus_plot[i] = np.std(cycles_T_minus_plot[i])
            self.mean_T_plus_plot[i] = np.median(cycles_T_plus_plot[i])
            self.std_T_plus_plot[i] = np.std(cycles_T_plus_plot[i])
            self.mean_A_minus_plot[i] = np.median(cycles_A_minus_plot[i])
            self.std_A_minus_plot[i] = np.std(cycles_A_minus_plot[i])
            self.mean_A_plus_plot[i] = np.median(cycles_A_plus_plot[i])
            self.std_A_plus_plot[i] = np.std(cycles_A_plus_plot[i])
